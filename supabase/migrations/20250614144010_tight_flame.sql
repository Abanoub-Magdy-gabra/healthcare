...
-- Add some audit log entries for the new data (fixed JSON construction)
DO $$
DECLARE
  admin_user_id uuid;
...
BEGIN
  ...
  -- Insert audit log if we have an admin user
  IF admin_user_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      NULL,
      jsonb_build_object(
        'action', 'Added comprehensive sample data for testing',
        'timestamp', current_timestamp_text,
        'description', 'Created additional appointments, medical records, payments, nurse requests, and messages for testing'
      ),
      '127.0.0.1'::inet
    );
  END IF;
