/*
  ...
  1. Sample Data
    - Add more rooms for testing
    - Add sample data that doesn't require specific user IDs
    - Create helper functions for when demo users actually register
  
  2. Notes
    - Demo user profiles will be created when users actually register through the auth system
    - This migration only creates supporting data and helper functions
*/
...
-- Create a function to set up demo data for specific user roles
...
BEGIN
  ...
  IF user_email IN (
    ...
    CASE user_role
      ...
      WHEN 'doctor' THEN
        -- Doctor gets some sample appointments (will be created when patients book)
...
BEGIN
  ...
  -- Call the original sample data function for other accounts
...
-- Create a view to show available demo accounts (for documentation)
CREATE OR REPLACE VIEW demo_accounts_info AS
SELECT 
  'admin@healthcareportal.com' as email,
  'admin123' as password,
  'admin' as role,
  'System Administrator' as description
UNION ALL
SELECT 
...
SELECT 
  ...
  'nurse' as role,
  'Nurse Emily Davis - Emergency Care' as description
UNION ALL
SELECT 
...
-- Add a comment explaining how to use demo accounts
...
-- Create some additional sample messages that will be available once demo users exist
